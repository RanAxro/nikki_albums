...
List<int> getAllWindowHandle() {
  ...
  final NativeCallable<WNDENUMPROC> callback =
      NativeCallable<WNDENUMPROC>.isolateLocal(
        enumWindowsCallback,
        exceptionalReturn: 0,
      );
...
(int, int, int) getDiskFreeSpaceEx(Path path) {
  if (Platform.isWindows) {
    ...
    try {
      final int res = GetDiskFreeSpaceEx(
        path.path.toNativeUtf16(),
        pAvailable,
        pTotal,
        pFree,
      );
...
Future<int> compressInWindows(
  ...
  final Process process = await Process.start(bin7z.path, [
    "a",
    "-tzip",
    "-mx=0",
    to.path,
...
Future<int> decompressInWindows(Path zipPath, Path to) async {
  ...
  final ProcessResult process = await Process.run(bin7z.path, [
    "x",
    zipPath.path,
    "-o${to.path}",
    "-y",
...
Future<String?> getWindowsUserName() async {
  try {
    ...
    if (result.exitCode != 0) {
      AppState.writeError(
        "utils.system.windows.getWindowsUserName",
