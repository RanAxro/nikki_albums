...
class Path {
  ...
  String get subName {
    ...
    return name.substring(
      0,
      name.length - (extensionLength == 0 ? 0 : extensionLength + 1),
    );
  }
  ...
  Future<void> copyDirectoryToDirectory(
    Path destination, {
  ...
  Future<void> moveDirectoryToDirectory(
    Path destination, {
    bool ignoreExists = false,
  }) async {
  ...
  Future<ProcessResult> generateDirectoryMklink(
    Path destination, {
    String? dirName,
    bool ignoreExists = false,
  }) {
    return Process.run("cmd", [
      ...
      "mklink",
      "/D",
      (destination + (dirName ?? name)).path,
      _path,
    ]);
    // return Process.run("cmd", ["/c", "mklink", "/D", "\"${(destination + (dirName ?? name)).path}\"", "\"$_path\""]);
  }

  Future<bool> generateDirectoryMklinkAsAdmin(
    Path destination, {
    String? dirName,
    bool ignoreExists = false,
  }) {
    return runWindowsCommandAsAdmin([
      [
        "mklink",
        "/D",
        "\"${(destination + (dirName ?? name)).path}\"",
