...
/// zip
Future<void> compressZip(
  List<File> files,
  List<Path> archiveFilePaths,
  File to, [
  ...
  try {
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      final ArchiveFile archiveFile = ArchiveFile.stream(
        archiveFilePaths[i].path,
        inputStream,
      );

      /// keep Original
      archiveFile.lastModTime =
          files[i].lastModifiedSync().millisecondsSinceEpoch ~/ 1000;
    ...
    encoder.encodeStream(
      ...
      callback: (ArchiveFile archiveFile) async {
        current += lastSize;
        lastSize = 1;
...
Future<void> compressZipIo(
  ...
  final isolate = await Isolate.spawn(_compressZipIsolate, [
    receivePort.sendPort,
    files.map((File file) => file.path).toList(),
    archiveFilePaths.map((Path path) => path.path).toList(),
    to.path,
  ]);
...
Future<void> _compressZipIsolate(List message) async {
  ...
  final List<File> files = (message[1] as List<String>)
      .map((String path) => File(path))
      .toList();
  final List<Path> archiveFilePaths = (message[2] as List<String>)
      .map((String path) => Path(path))
      .toList();
  ...
  compressZip(
    ...
    archiveFilePaths,
    to,
    (double progress) => sendPort.send(progress),
  );
}

Future<void> compressZipIoX(
  List<File> files,
  List<Path> archiveFilePaths,
  File to, {
  int core = 6,
  ...
  final List<List<Path>> archiveFilePathsSplit = splitEvenly<Path>(
    archiveFilePaths,
    core,
  );
  ...
  for (int i = 0; i < filesSplit.length; i++) {
    ...
    compressZipIo(filesSplit[i], archiveFilePathsSplit[i], singleZip, (
      double theIoProgress,
    ) {
      process[i] = theIoProgress;
      final double compressProgress =
          process.reduce((a, b) => a + b) / process.length;
...
Future<void> mergeZipArchive(
  ...
  encoder.encodeStream(
    ...
    callback: (ArchiveFile archiveFile) {
      ...
      archiveFile.lastModTime =
          _getLastModDateTime(archiveFile.lastModTime).millisecondsSinceEpoch ~/
          1000;
...
Future<void> mergeZipArchiveIo(
  ...
  final isolate = await Isolate.spawn(_mergeZipArchiveIsolate, [
    receivePort.sendPort,
    zipsSplit.map((File singleZip) => singleZip.path).toList(),
    to.path,
  ]);
...
Future<void> _mergeZipArchiveIsolate(List message) async {
  final SendPort sendPort = message[0];
  final List<File> zipsSplit = (message[1] as List<String>)
      .map((String path) => File(path))
      .toList();
...
Future<void> decompressZipIo(
  ...
  final isolate = await Isolate.spawn(_decompressIsolate, [
    receivePort.sendPort,
    zip.path,
    to.path,
  ]);
