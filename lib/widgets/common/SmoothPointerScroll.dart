...
class SmoothPointerScroll extends StatefulWidget {
  ...
  builder; // common 返回 ScrollController 传入到需要的 ScrollView 上
...
class _SmoothPointerScroll extends State<SmoothPointerScroll> {
  ...
  late final IndependentScrollbarController
  scrollbarController; // 滚动条 Controller
  ...
  void initState() {
    ...
    scrollbarController =
        widget._scrollbarController ?? IndependentScrollbarController();
    ...
    scrollbarController.addListener(() {
      if (scrollbarController.isDrag) {
        ...
        forceOffset =
            scrollController.position.maxScrollExtent *
            scrollbarController.progress;
      }
    });
    scrollController.addListener(() {
      if (!scrollController.hasClients) return;

      scrollbarController.virtualScrollViewExtent =
          scrollController.position.maxScrollExtent;
