    ...
    await Future.wait([
      ...
      compressZipIoX(
        ...
        onProcess: (double encodeProgress) {
          ...
          progress.value =
              progressDetail.reduce((a, b) => a + b) / progressDetail.length;
        },
      ),
      // encode nikkias
...
class _SendFileBuilderState extends State<SendFileBuilder> {
  ...
  Response getFileListResponse(Request request) {
    ...
    final List<String> names = widget.files
        .map((File file) => Path(file.path).name)
        .toList();
  ...
  Response downloadArchiveResponse(Request request) {
    ...
    final int index =
        int.tryParse(
          request.requestedUri.queryParameters["index"].toString(),
        ) ??
        0;
    ...
    return Response.ok(
      ...
      headers: {
        ...
        HttpHeaders.contentDisposition:
            'attachment; filename="${Path(archive.path).name}"',
  ...
  Future<Response> downloadFileResponse(Request request) async {
    ...
    final int index = widget.files.indexWhere(
      (File file) => Path(file.path).name == filename,
    );
    if (index == -1) return Response.notFound("Not Found");

    final Path path = Path(widget.files[index].path).cut(1);

    final Response res = await createStaticHandler(
      path.path,
    ...
    return res.change(
      headers: isPreview
          ...
          : {
              ...
              HttpHeaders.contentLengthHeader: widget.files[index]
                  .lengthSync()
                  .toString(),
            },
    );
  ...
  Future<TransmissionInfo> startServer() async {
    ...
    final TransmissionInfo info = TransmissionInfo(
      code: code,
      ipv4: ip,
      port: port,
    );
    ...
    print(
      'Serving at http://${await NetworkIPHelper.getActiveNetworkIP() ?? "127.0.0.1"}:$port',
    );
