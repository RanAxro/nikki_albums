      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: AppIcon("big/receiveFile", height: null),
          ),
              ...
              AppButton(
                ...
                onClick: () async {
                  final File? downloadFile = await downloadFileByUrl(
                    context,
                    controller.text,
                    await getTempPath() + "receive.zip",
                  );
                  if (downloadFile != null && context.mounted) {
                    parseNikkiasFile(context, downloadFile);
                  }
                },
                child: AppIcon("forward", height: 24),
              ),
            ],
          ),
...
class UdpListenerBuilder extends StatefulWidget {
  ...
  const UdpListenerBuilder({super.key, required this.builder});
    ...
    socket.listen((RawSocketEvent event) {
      ...
      items.add(
        UdpTransmissionInfoWrapper(
          ...
          onDie: () {
            items.removeWhere(
              (UdpTransmissionInfoWrapper item) => item.info == info,
            );
            setState(() {});
          },
        ),
