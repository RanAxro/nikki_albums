      ...
      child: Align(
        ...
        child: SizedBox(
          ...
          child: Column(
            ...
            children: [
              Text(
                "${context.tr("version")}: $versionString",
                style: TextStyle(
          ...
          RFutureBuilder(
            ...
            builder: (BuildContext context, UpdateInfo? info) {
              ...
              if (info.version <= version)
                return Text(
                  context.tr("alreadyTheLatestVersion"),
                  style: TextStyle(
                    color: AppTheme.of(context)!.colorScheme.background.onColor,
                  ),
                );
              ...
              return Text(
                "${context.tr("findNewVersion")} ${info.versionString}",
                style: TextStyle(
                  color: AppTheme.of(context)!.colorScheme.background.onColor,
                ),
              );
...
Future<UpdateInfo?> checkForUpdates() async {
  try {
    ...
    return UpdateInfo(
      ...
      versionString: info["versionString"] is String
          ? info["versionString"]
          : info["version"].toString(),
      downloadLink: info["downloadLink"] is String ? info["downloadLink"] : "",
      updateMessage: info["updateMessage"] is String
          ? info["updateMessage"]
          : "",
    );
  } catch (e) {
...
class UpdateDialog extends StatelessWidget {
  ...
  Future<void> _download(BuildContext context) async {
    if (info.downloadLink == "") {
      ...
      // decompress
      final Path? decompressPath = AppState.sfxPath.value == null
          ? getWindowsDesktopPath()
          : Path(AppState.sfxPath.value!);
      bool isDecompress = false;

      String? exeFilename;

      if (decompressPath != null) {
        final InputFileStream inputStream = InputFileStream(
          archiveSavePath.path,
        );
        try {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              "${context.tr("checkForUpdates")}: ${info.versionString} !",
              style: TextStyle(
                color: AppTheme.of(context)!.colorScheme.background.onColor,
              ),
            ),
