...
Future<void> buildPlugin(PluginBuilderConfig config, Directory output) async {
  ...
  final Uint8List infoBytes = await serializePluginInfo(
    value: config.info,
    pretty: true,
  );
  ...
  for (final MapEntry<String, dynamic> langEntry
      in config.languageList.entries) {
    final String jsonStr = jsonEncode(langEntry.value);
    await File(
      p.join(pluginPath, "lang", "${langEntry.key}.json"),
  ...
  for (final GameConfig gameConfig in config.gameConfigList) {
    final Uint8List gameConfigBytes = await serializeGameConfig(
      value: gameConfig,
      pretty: true,
    );
    await File(
      p.join(pluginPath, "game_config", "${gameConfig.id}.json"),
    ).create(recursive: true);
    await File(
      p.join(pluginPath, "game_config", "${gameConfig.id}.json"),
    ).writeAsBytes(gameConfigBytes);
  }
}
