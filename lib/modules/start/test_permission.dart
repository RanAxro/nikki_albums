...
abstract class TestPermission {
  static Future<bool> mklink(Path test) async {
    ...
    try {
      final ProcessResult result = await test.generateDirectoryMklink(
        test.cut(1),
        dirName: ms,
      );
      print(result.stderr);
  ...
  static Future<bool> mklinkAsAdmin(Path test) async {
    ...
    final bool res = await test.generateDirectoryMklinkAsAdmin(
      test.cut(1),
      dirName: ms,
    );
    ...
    if (res) {
      ...
      while (tryTimes-- != 0) {
        try {
          ...
          if (tryTimes == 0) {
            AppState.writeError(
              "star.testPermission.mklinkAsAdmin.delete",
              e.toString(),
            );
          }
        }
