...
final ContentItem item = ContentItem(
  name: "start",
  icon: AppIcon("run", height: mediumButtonContentSize),
  page: const Start(),
);
            ...
            showDialog(
              ...
              builder: (BuildContext context) {
                return MoveFolderDialog(
                  game: AppState.currentGame.value!,
                  folder: AppState.currentGame.value!.gamePlayPhotosPath,
                );
              },
            );
          },
          ...
          onPressed: () {
            ...
            showDialog(
              ...
              builder: (BuildContext context) {
                return MoveFolderDialog(
                  game: AppState.currentGame.value!,
                  folder: AppState.currentGame.value!.installPath,
                );
              },
            );
          },
...
class MoveFolderDialog extends StatelessWidget {
  ...
  MoveFolderDialog({super.key, required this.game, required this.folder});
    ...
    final List<Widget> ui = [
      ...
      StatefulBuilder(
        builder:
            (
              ...
              void Function(void Function()) reTestPermission,
            ) {
      ...
      ValueListenableBuilder(
        ...
        builder: (BuildContext context, String? error, Widget? child) {
          ...
          return Text(
            error == "existXInDestination"
                ? context.tr(error, args: [folder.name])
                : context.tr(error),
            style: TextStyle(
              color: AppTheme.of(context)!.colorScheme.error.pressedColor,
            ),
...
class _BackupAlbumProcessorState extends State<BackupAlbumProcessor> {
  final ManualValueNotifier<List<Uid>> processedUid =
      ManualValueNotifier<List<Uid>>([]);
  bool init = false;
  ...
  Future<void> startExport(BuildContext context) async {
    ...
    final AlbumBackupNikkiasManifest manifest = AlbumBackupNikkiasManifest(
      launcherChannel: widget.game.launcherChannel,
    );

    try {
      final AlbumBackupNikkiasCodec codec = AlbumBackupNikkiasCodec(
        manifest,
        savePath.file,
        widget.game.installPath,
      );
      codec.uidWhitelist = processedUid.value
          .map((Uid uid) => uid.value)
          .toList();
      await codec.encode(
