...
class GameSelectorBuilder extends StatelessWidget {
  ...
  final Widget Function(
    ...
    Future<List<Game>> foundGameList,
    List<Game> customGameList,
  )
  builder;

  const GameSelectorBuilder({super.key, this.whitelist, required this.builder});

  List<Game> _filter(List<Game> raw) {
    if (whitelist == null) return raw;

    return raw
      ..removeWhere((Game game) => !whitelist!.contains(game.launcherChannel));
  }
...
class UidSelectorBuilder extends StatelessWidget {
  final Game game;
  final Widget Function(BuildContext context, Future<List<Uid>> uidList)
  builder;

  const UidSelectorBuilder(this.game, {super.key, required this.builder});

  @override
  Widget build(BuildContext context) {
    return builder(context, game.findUid());
  }
...
//   final Widget Function(BuildContext context, Widget gameBox) common;
...
class AccountSelector {
  ...
  const AccountSelector({
    ...
    this.onAdd,
       ...
       assert(
         !(game != null && whitelist != null),
         "错误: game 与 whitelist 至少有一个为 null",
       ),
       assert(
         !(game != null && isSelectUid == false),
         "错误: game 不为 null 时, isSelectUid 必须为 true",
       ),
       assert(
         !(isSelectUid == false && mustSelectUid == true),
         "错误: mustSelectUid 为 true 时, isSelectUid 必须为 true",
       );
    ...
    return SubmenuButton(
      menuStyle: MenuStyle(
        backgroundColor: WidgetStateProperty.all(
          AppTheme.of(context)!.colorScheme.byRole(colorRole).color,
        ),
      ),
  ...
  List<Widget> _menuChildrenBuilder(BuildContext context, ColorRole colorRole) {
    ...
    menuChildren.add(
      RFutureBuilder<Map<Game, List<Uid>>>(
        ...
        builder: (BuildContext context, Map<Game, List<Uid>> games) {
          final List<MapEntry<Game, List<Uid>>> gameEntries = games.entries
              .toList();

          return Column(
            children: List.generate(
              gameEntries.length,
              (int index) => _itemBuilder(
                context,
                colorRole,
                gameEntries[index].key,
                gameEntries[index].value,
              ),
            ),
          );
        },
        ...
        MenuItemButton(
          ...
          child: Text(
            ...
            style: TextStyle(
              color: AppTheme.of(
                context,
              )!.colorScheme.byRole(colorRole).onColor,
            ),
          ),
        ),
      );
