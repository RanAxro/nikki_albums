...
class UidNote {
  ...
  bool hasSame(Iterable<UidNote> iterable) {
    return iterable.any(
      (UidNote test) => installPath == test.installPath && value == test.value,
    );
  }
...
class Uid {
  static Map<String, String?>? toJsonMap(Uid? uid) {
    ...
    return {"installPath": uid.installPath.path, "value": uid.value};
  }
  ...
  static Uid? from(dynamic map) {
    ...
    final Uid res = Uid(installPath: installPath, value: map["value"]);
    return res;
  }
  ...
  Uid({required this.installPath, required this.value}) {
    updateAvatar();
  }
  ...
  set note(String? note) {
    ...
    final UidNote? newNote = note == null
        ? null
        : UidNote(note: note, installPath: installPath, value: value);

    AppState.uidNotes.value = {
      ?newNote,
      ...AppState.uidNotes.value..remove(oldNote),
    };
  }
  ...
  Future<Path?> _getAvatar() async {
    final Directory avatarDirectory =
        (installPath +
                albumsInfoMap[AlbumType.CustomAvatar]!.locateInGame.replaceAll(
                  r"$uid$",
                  value,
                ))
            .directory;
    ...
    final List<FileSystemEntity> entities = await avatarDirectory
        ...
        .where(
          (entity) => entity is File && isImageExtension(Path(entity.path)),
        )
        .toList();

    if (entities.isEmpty) return null;

    final List<Path> avatarList = entities
    ...
    avatarList.sort(
      (a, b) => b.cacheStat.modified.compareTo(a.cacheStat.modified),
    );
...
extension FindUid on Uid {
  static bool isUidType(String source) {
    return RegExp(r"^\d{6,12}$").hasMatch(source);
  }

  static Future<List<Uid>> find(Path installPath) async {
    ...
    for (AlbumsInfoItem info in albumsInfoMap.values) {
      ...
      try {
        ...
        /// @ 1
        AppState.writeError("uid.FindUid.find.@1", e.toString());
        continue;
      }
    }
