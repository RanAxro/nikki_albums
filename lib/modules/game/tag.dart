...
class PresetTagGroup extends TagGroup {
  ...
  const PresetTagGroup({Set<String>? tags, super.listener})
    : super(color: defaultColor, tags: tags ?? const <String>{});
}
...
class CustomTagGroup extends TagGroup {
  ...
  const CustomTagGroup({
    ...
    Set<String>? tags,
    super.listener,
  }) : super(color: color ?? defaultColor, tags: tags ?? const <String>{});
}
...
class TagBox extends ChangeNotifier {
  ...
  factory TagBox(Iterable<TagGroup> groups) {
    ...
    return TagBox._(presetGroup: presetGroup, customGroups: customGroups);
  }
  ...
  CustomTagGroup setCustomTag(
    ...
    if (maybeGroup == null) {
      _customGroups[setName] = CustomTagGroup(
        name: setName,
        color: color,
        tags: tags,
      ...
      _customGroups[setName] = CustomTagGroup(
        name: setName,
        color: color ?? maybeGroup.color,
        tags: tags ?? maybeGroup._tags,
      )..addListener(notifyListeners);
...
abstract class TagCodec {
  ...
  const TagCodec({required this.version});
...
class Tag extends ChangeNotifier {
  ...
  static TagBoxType defaultBox = [
    {"name": null, "color": Colors.orange.toARGB32(), "tags": []},
  ];
  ...
  Future<void> init() async {
    try {
      if (!await tagPath.file.exists()) {
        ...
        _tagBox = List<Map<String, dynamic>>.from(
          jsonDecode(await tagPath.file.readAsString()),
        );
      }
    } catch (e) {
      _tagBox = defaultBox;
  ...
  void delete(String tag) {
    ...
    _tagBox!.removeWhere(
      (item) => item.containsKey("name") && item["name"] == tag,
    );
