...
final ContentItem item = ContentItem(
  name: "recycle_bin",
  icon: AppIcon("recycle_bin", height: mediumButtonContentSize),
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Expanded(child: DeletionBox(records)),
                      ],
                    ),
                  ),
                );
          ...
          (
            ...
            return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                final String typeStr = deletionRecord.deletionAlbumType
                    .map(
                      (AlbumType type) => context.tr(albumsInfoMap[type]!.name),
                    )
                    .join(" | ");
                            ...
                            Row(
                              ...
                              children: [
                                AppKVText(
                                  ...
                                  v: deletionRecord.uid,
                                ),
                                AppKVText(
                                  k: "quantity",
                                  v: deletionRecord.quantity.toString(),
                                ),
                                AppKVText(k: "type", v: typeStr),
                              ],
                            ),
                          ],
                        ),
...
class DeletionRecord {
  ...
  AlbumType? get suggestedAlbumType {
    ...
    return deletionAlbumType
            .where((AlbumType type) => type != AlbumType.NikkiPhotos_LowQuality)
            .firstOrNull ??
        deletionAlbumType.firstOrNull;
  }
  ...
  Path getAlbumRecycleBinPath() {
    return installPath +
        Path(locateToRecycleBin) +
        deletionTime.millisecondsSinceEpoch.toString();
  }

  Future<void> restore({
    void Function(double progress)? onProgress,
    void Function(int successes, int failures)? onFinish,
  }) async {
    int total = 0;
    int errorNum = 0;
...
class _RecycleBinBuilderState extends State<RecycleBinBuilder> {
  ...
  Future<List<DeletionRecord>> getDeletionRecord() async {
    ...
    ///                          ===========
    await for (final FileSystemEntity entityNamedMsSinceEpoch
        in recycleBinDir.directory.list()) {
      ...
      final DateTime deletionTime = DateTime.fromMillisecondsSinceEpoch(
        msSinceEpoch,
      );
      String? uid;
      ...
      ///  \NikkiAlbumsRecycleBin\$msSinceEpoch$\$albumType$\[$uid$]
      ///                                         =========
      await for (final FileSystemEntity entityNamedAlbumType
          in entityNamedMsSinceEpoch.list()) {
        if (entityNamedAlbumType is! Directory) continue;

        final AlbumType? albumType = AlbumType.fromStrictly(
          Path(entityNamedAlbumType.path).name,
        );
        ...
        ///                                                     ====
        if (albumsInfoMap[albumType]!.isRequireUid) {
          await for (final FileSystemEntity entityNamedUid
              in entityNamedAlbumType.list()) {
            ...
            quantity = max(
              quantity,
              (await entityNamedUid.list().toList()).length,
            );
          }
        } else {
          quantity = max(
            quantity,
            (await entityNamedAlbumType.list().toList()).length,
          );
        }
      }
...
class DeleteDirectoryProcessor extends StatefulWidget {
  ...
  @override
  State<DeleteDirectoryProcessor> createState() =>
      _DeleteDirectoryProcessorState();
}
