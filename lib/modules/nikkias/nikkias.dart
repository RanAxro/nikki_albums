...
abstract class NikkiasCodec {
  ...
  Future<File> _prepareEncode() async {
    ...
    final File manifestFile =
        (Path(nikkiasFile.path).cut(1) +
                ("${Path(nikkiasFile.path).name}.manifest"))
            .file;
...
class AlbumBackupNikkiasCodec extends NikkiasCodec {
  ...
  AlbumBackupNikkiasCodec(
    AlbumBackupNikkiasManifest super.manifest,
    super.nikkiasFile,
    this.installPath,
  );
  ...
  Future<void> decode(void Function(double)? onProgress) async {
    ...
    /// decode
    await decompressZipIo(nikkiasFile, installPath.directory, (
      double decompressProgress,
    ) {
  ...
  Future<void> encode(void Function(double)? onProgress) async {
    ...
    for (final MapEntry<AlbumType, AlbumsInfoItem> info
      ...
      if (info.value.isRequireUid) {
        for (final Uid uid in uidList) {
          final List<File> imageFiles = await listDirFile(
            (installPath +
                    info.value.locateInGame.replaceAll(r"$uid$", uid.value))
                .directory,
          );
          final List<Path> imageArchiveFilePaths = imageFiles
              .map(
                (File file) =>
                    Path(file.path.substring(installPath.path.length + 1)),
              )
              .toList();
          ...
          if (info.value.locateInBackup != null) {
            final List<File> backupImageFiles = await listDirFile(
              (installPath +
                      info.value.locateInBackup!.replaceAll(
                        r"$uid$",
                        uid.value,
                      ))
                  .directory,
            );
            final List<Path> backupImageArchiveFilePaths = backupImageFiles
                .map(
                  (File file) =>
                      Path(file.path.substring(installPath.path.length + 1)),
                )
        ...
        final List<File> imageFiles = await listDirFile(
          (installPath + info.value.locateInGame).directory,
        );
        final List<Path> imageArchiveFilePaths = imageFiles
            .map(
              (File file) =>
        ...
        if (info.value.locateInBackup != null) {
          final List<File> backupImageFiles = await listDirFile(
            (installPath + info.value.locateInBackup!).directory,
          );
          final List<Path> backupImageArchiveFilePaths = backupImageFiles
              .map(
                (File file) =>
    ...
    await compressZipIoX(
      files,
      archiveFilePaths,
      nikkiasFile,
      onProcess: (double decompressProgress) {
        onProgress?.call(decompressProgress * 0.99);
      },
    );
...
class ImageTransferNikkiasCodec extends NikkiasCodec {
  ...
  ImageTransferNikkiasCodec(
    ImageTransferNikkiasManifest super.manifest,
    super.nikkiasFile,
    this.installPath,
  );
  ...
  Future<void> decode(void Function(double)? onProgress) async {
    ...
    final AlbumsInfoItem albumInfo =
        albumsInfoMap[(manifest as ImageTransferNikkiasManifest).albumType]!;
    final Path albumPath =
        installPath +
        (albumInfo.isRequireUid
            ? albumInfo.locateInGame.replaceAll(
                r"$uid$",
                (manifest as ImageTransferNikkiasManifest).uid,
              )
            : albumInfo.locateInGame);

    /// decode
    await decompressZipIo(nikkiasFile, albumPath.directory, (
      double decompressProgress,
    ) {
      onProgress?.call(decompressProgress * 0.99);
    });
  ...
  Future<void> encode(void Function(double)? onProgress) async {
    ...
    if (filenameWhitelist != null) {
      imageFiles.removeWhere(
        (File file) => !filenameWhitelist!.contains(Path(file.path).name),
      );
    }
    final List<Path> imageArchiveFilePaths = imageFiles
        .map((File file) => Path(Path(file.path).name))
        .toList();
    files.addAll(imageFiles);
    archiveFilePaths.addAll(imageArchiveFilePaths);
...
class OtherNikkiasCodec extends NikkiasCodec {
  ...
  OtherNikkiasCodec(
    OtherNikkiasManifest super.manifest,
    super.nikkiasFile,
    this.saveDir,
  );
...
abstract class Nikkias {
  ...
  static Future<void> registerFileAssociation() async {
    ...
    try {
      ...
      final RegistryKey extKey = hkcu.createKey(
        "Software\\Classes\\$nikkiasWholeExtension",
      );
      extKey.createValue(RegistryValue.string("", nikkiasFileProgId));
      extKey.close();

      // 创建 ProgID 主键
      final RegistryKey progKey = hkcu.createKey(
        "Software\\Classes\\$nikkiasFileProgId",
      );
