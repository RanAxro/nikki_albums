...
final ContentItem item = ContentItem(
  name: "resource",
  icon: AppIcon("resource", height: mediumButtonContentSize),
  page: Resource(),
);
...
class Resource extends StatelessWidget {
  ...
  final ValueNotifier<ResourceType> resourceType = ValueNotifier(
...
class ResourceButton extends StatelessWidget {
  ...
  const ResourceButton({super.key, required this.resourceType});
...
class ResourceViewer extends StatelessWidget {
  final ValueNotifier<ResourceType> resourceType;
  const ResourceViewer({super.key, required this.resourceType});
        ...
        if (type == ResourceType.Movies) {
          return RFutureBuilder(
            future: Future(() async {
              ...
              try {
                final list = await path.directory
                    .list(recursive: true)
                    .toList();
            ...
            try {
              ...
              for (final item in list) {
                ...
                if (isImageExtension(path) &&
                    FileSystemEntityType.file == await path.typeAsync) {
                  res.add(path);
                }
              }
...
class GridViewer extends StatelessWidget {
  ...
  const GridViewer({super.key, required this.images});
...
class ListViewer extends StatelessWidget {
  ...
  const ListViewer({super.key, required this.movies});
...
class Exhibit extends StatefulWidget {
  ...
  const Exhibit(this.images, this.currentImage, {super.key});
...
class _ExhibitState extends State<Exhibit> {
  ...
  Widget build(BuildContext context) {
    return RFutureBuilder(
      future: ImageThumbnail.fromCache(
        id: widget.currentImage.name,
