...
impl<F: Formatter, T: FTransformSerializer> Serializer<F, T>{
  #[inline]
  fn serialize(&mut self, value: &FTransforms) -> Output{
    ...
    for transform in value{
      self.transform_serializer.serialize(&mut self.formatter, &mut output, transform);
    }
...
trait FTransformSerializer{
  fn serialize<F: Formatter>(&mut self, formatter: &mut F, output: &mut Output, value: &FTransform);
  #[inline]
  fn serialize_quat<F: Formatter>(&mut self, formatter: &mut F, output: &mut Output, value: &FQuat){
  ...
  #[inline]
  fn serialize_vector<F: Formatter>(&mut self, formatter: &mut F, output: &mut Output, value: &FVector){
    formatter.write_f64(output, value.x);
    formatter.write_f64(output, value.y);
    formatter.write_f64(output, value.z);
  }
...
fn test(){
  let transforms = vec![
    FTransform{
      rotation: FQuat{x: 2.65, y: 4.35, z: -10.22, w: 1.0},
  ...
  // 0100000033333333333305406666666666661140713D0AD7A37024C0000000000000F03F91ED7C3F153BD040EE7C3F355E2A664066666666666CA540000000000000F03F000000000000F03F000000000000F03F
  println!("{}", res);
}