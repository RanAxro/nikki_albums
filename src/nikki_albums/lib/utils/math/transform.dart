...
Point<double> rotatePoint(
  ...
  final double angle = unit == AngleUnit.radian
      ? rotation
      : convertAngleUnits(rotation, AngleUnit.degree, AngleUnit.radian);
...
Size getRotatingRectangleBoundary(
  Size rect,
  double rotation, {
  AngleUnit unit = AngleUnit.radian,
}) {
...
double fitRotatingRectangleScale(
  Size boundary,
  Size adapted, {
  FitMode mode = FitMode.contain,
  double boundaryRotation = 0,
  double adaptedRotation = 0,
  AngleUnit unit = AngleUnit.radian,
}) {
  final Size rotatedAdaptedRect = getRotatingRectangleBoundary(
    adapted,
    adaptedRotation - boundaryRotation,
    unit: unit,
  );
...
Offset centerRotatingRectangle(
  Size boundary,
  Size centered, {
  double boundaryRotation = 0,
  double centeredRotation = 0,
  AngleUnit unit = AngleUnit.radian,
}) {
  ...
  final Size rotatedCentered = getRotatingRectangleBoundary(
    centered,
    centeredRotation,
    unit: unit,
  );
...
Offset transformPointRelativeToRotatingRectangle(
  Size rect, {
  double rotation = 0,
...
double fitRotatingRectangleScaleWithReferencePoint(
  Size boundary,
  Size adapted, {
  Offset relativeToBoundary = Offset.zero,
  Offset relativeToAdapted = Offset.zero,
  double boundaryRotation = 0,
  double adaptedRotation = 0,
  AngleUnit unit = AngleUnit.radian,
}) {
  ...
  // angle = angle % (2 * pi) - pi;
  //
  final Size rotatingAdaptedBoundary = getRotatingRectangleBoundary(
    adapted,
    angle,
  ...
  final Offset relativeToRotatingAdaptedBoundary =
      transformPointRelativeToRotatingRectangle(
        adapted,
        rotation: angle,
        point: relativeToAdapted,
        unit: unit,
      );

  final double leftScale =
      relativeToBoundary.dx / relativeToRotatingAdaptedBoundary.dx;
  final double topScale =
      relativeToBoundary.dy / relativeToRotatingAdaptedBoundary.dy;
  final double rightScale =
      (boundary.width - relativeToBoundary.dx) /
      (rotatingAdaptedBoundary.width - relativeToRotatingAdaptedBoundary.dx);
  final double bottomScale =
      (boundary.height - relativeToBoundary.dy) /
      (rotatingAdaptedBoundary.height - relativeToRotatingAdaptedBoundary.dy);
