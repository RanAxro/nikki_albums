  ...
  bool _filterItem(ImageItem item){
    ...
    if(_filtration.contains(Filtration.onlyDailyTask)){
      ...
      if(item.isObtainedAddition()){
        final Field field = item.getAdditionSync(uid?.value, AlbumType.NikkiPhotos_HighQuality);

        final List<Field>? taskFieldList = field["ia_photography_info"].firstOrNull?["ia_task"].firstOrNull?["ia_task_photo"].firstOrNull?.children;
        ...
        if(photoRegionTaskList == null || photoRegionTaskList.isEmpty) return false;
