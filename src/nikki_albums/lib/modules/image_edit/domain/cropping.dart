...
import "dart:math";
import "package:nikkialbums/utils/math/angle.dart";
...
enum CropFrameAnchorPoint{
  ...
  Vector4 get frag{
    if(!_fragMap.containsKey(this)){
      _fragMap[this] = Vector4(tl.toDouble(), tt.toDouble(), tr.toDouble(), tb.toDouble());
    }
...
class ImageCroppingParamDef<T>{
  static const ImageCroppingParamDef rotation = ImageCroppingParamDef<double>(ImageCroppingType.rotation, "ie_rotation", 0);
  static const ImageCroppingParamDef cropFrameRatio = ImageCroppingParamDef<CropFrameRatio>(ImageCroppingType.cropFrameRatio, "ie_crop_frame_ratio", CropFrameRatio.free);
  static const ImageCroppingParamDef cropFrameLeft = ImageCroppingParamDef<double>(ImageCroppingType.cropFrameLeft, "ie_crop_frame_left", 0);
  static const ImageCroppingParamDef cropFrameTop = ImageCroppingParamDef<double>(ImageCroppingType.cropFrameTop, "ie_crop_frame_top", 0);
  static const ImageCroppingParamDef cropFrameRight = ImageCroppingParamDef<double>(ImageCroppingType.cropFrameRight, "ie_crop_frame_right", 1);
  static const ImageCroppingParamDef cropFrameBottom = ImageCroppingParamDef<double>(ImageCroppingType.cropFrameBottom, "ie_crop_frame_bottom", 1);
...
class ImageCroppingHandler{
  ...
  Future<ui.Image> handle(ImageCroppingParams params) async{
    ...
    final Size rotatedBoundary = getRotatingRectangleBoundary(originalSize, params.rotation, unit: AngleUnit.radian);
...
class ImageCroppingProcessor{
  ...
  ImageCroppingProcessor(int rotation, double width, double height, CropFrameRatio ratio, this.minLength, Vector4 frame) :
    _rotation = rotation,
    _width = width,
    _height = height,
    _ratio = ratio,
  ...
  factory ImageCroppingProcessor.fromLTRB({
    ...
    return ImageCroppingProcessor(0, width, height, ratio, max(0, minLength), Vector4(l, t, r, b));
  }
  ...
  factory ImageCroppingProcessor.fromParams({
    ...
    return ImageCroppingProcessor(
      ...
      minLength,
      Vector4(params.cropFrameLeft * width, params.cropFrameTop * height, params.cropFrameRight * width, params.cropFrameBottom * height),
    );
  }
  ...
  static Vector4 getIntersection(double width, double height, Vector4 rect){
    ...
    if(rect.left > width || rect.top > height || rect.right < 0 || rect.bottom < 0){
      if(rect.left > width){
        return Vector4(width, rect.top.clamp(0, height), width, rect.bottom.clamp(0, height));
      }
      if(rect.top > height){
        return Vector4(rect.left.clamp(0, width), height, rect.right.clamp(0, width), height);
      }
      if(rect.right < 0){
        return Vector4(0, rect.top.clamp(0, height), 0, rect.bottom.clamp(0, height));
      }

      /// bottom < 0
      return Vector4(rect.left.clamp(0, width), 0, rect.right.clamp(0, width), 0);
    }
    // 包含
  ...
  CropFrameAnchorPoint getAnchorPoint(Vector2 pos, {double e = 5}){
    ...
    if(pos.x < left - e || pos.x > right + e || pos.y < top - e || pos.y > bottom + e) return CropFrameAnchorPoint.none;
  ...
  set params(ImageCroppingParams p){
    ...
    _frame.setValues(p.cropFrameLeft * width, p.cropFrameTop * height, p.cropFrameRight * width, p.cropFrameBottom * height);
    _frame.copyInto(_virtuality);
...
class ImageCroppingController extends ChangeNotifier{
  ...
  CropFrameAnchorPoint getAnchorPoint(Offset offset){
    ...
    return _processor.getAnchorPoint(Vector2(localOffset.dx / _scale, localOffset.dy / _scale), e: e / _scale);
  }
