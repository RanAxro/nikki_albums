...
class ImageEditorController extends ChangeNotifier implements EditStackController<ImageEditParams>{
  ...
  ImageEditorController({
    ...
    _plate = initPlate,
    _stack = EditStackController<ImageEditParams>(initParams ?? ImageEditParams.defaultParams);
...
class ImageEditHandler{
  ...
  Future<ui.Image> handle(ImageEditParams params) async{
    final ImageFragmentHandler fragmentHandler = ImageFragmentHandler(image: image);

    final ui.Image fragmentImage = await fragmentHandler.handle(params.fragment);

    final ImageCroppingHandler croppingHandler = ImageCroppingHandler(image: fragmentImage);

    final ui.Image croppingImage = await croppingHandler.handle(params.cropping);
