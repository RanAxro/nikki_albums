...
import "package:nikkialbums/modules/game/field_tree.dart";
import "package:nikkialbums/modules/image_edit/presentation/image_editor.dart";
import "album_view.dart";
import "album_previewer.dart";
import "package:nikkialbums/modules/file_transfer/file_transfer.dart";
...
import "package:nikkialbums/modules/game/album_manager.dart";
...
import "package:nikkialbums/modules/tutorial/tutorial.dart";
...
class AlbumValuePool extends InheritedWidget{
  ...
  static AlbumValuePool of(BuildContext context){
    return context.dependOnInheritedWidgetOfExactType<AlbumValuePool>() as AlbumValuePool;
  }
}
...
class _ToolBarState extends State<ToolBar>{
  ...
  Widget gameListenerBuilder(Game game, Widget Function(bool isExistSelectedImage, bool isAllowBackup) builder){
        ...
        return Row(
          children: [
            gameListenerBuilder(game, (bool isExistSelectedImage, bool isAllowBackup){
              ...
              return AppText(
                context.plural("pa_select_X_image", game.album.selectedImages.length),
                isTranslate: false,
              );
            }),
      ...
      children: [
        ...
        if(game != null)
          ListenableBuilder(
            ...
            builder: (BuildContext context, Widget? child){
              return controller.isAttach ? TagMenu(
                game: game!,
        ...
        if(game != null)
          ListenableBuilder(
            listenable: controller,
            builder: (BuildContext context, Widget? child){
              return controller.isAttach ? NotifierBuilder(
                listenable: game!.album.whenSelectedImagesChange,
                builder: (BuildContext context, Widget? child){
                  ...
                  final String icon = isSelected ? "assets/icon/select_all.webp" : "assets/icon/notSelect.webp";
                  return Tooltip(
                    ...
                    child: MultiSplitView(
                      initialAreas: [
                        ...
                        AppState.imageCustomDataWidgetSize.value == null ?
                          ...
                          Area(data: "data_panel", size: AppState.imageCustomDataWidgetSize.value),
                      ],
                            ...
                            builder: (BuildContext context, Widget? child){
                              return LayoutBuilder(
                                builder: (BuildContext context, BoxConstraints constraint){
                                  ...
                                  if(controller.isAttach){
                                    rootField = images[controller.index].getAdditionSync(game?.selectedUid?.value, game?.selectedAlbum);
                                  }else{
                                    rootField = initImage.getAdditionSync(game?.selectedUid?.value, game?.selectedAlbum);
                                  }
                                  ...
                                  return FieldTree(
                                    ...
                                    config: FieldTreeConfiguration(
                                      onFieldTap: (Field field) async{
                                        if(field.key == "ia_camera_params"){
                                          bool isError = false;

                                          try{
                                            await copyTextToClipboard(field.value.toString());
                                          }catch(e){
                                            isError = true;
                                          }
                                        ...
                                        else if(field.key == "ia_photo_region" || field.key == "ia_possible_photo_region"){
                                          final (GamePhotoRegion photoRegionInfo, num x, num y, num z) = field.value;

                                          final (int px, int py) = photoRegionInfo.map.coordToPixel((x, y));

                                          showAppDialog(context: context, builder: (context){
                                            return AppDialog(
                                              title: photoRegionInfo.stringData,
                                              child: GestureDetector(
                                                onSecondaryTap: (){
                                                  Navigator.of(context).pop();
                                                },
                                                child: AppMapViewer(
                                                  assetName: photoRegionInfo.map.assetName,
                                                  imageSize: Size(photoRegionInfo.map.mapSize.$1.toDouble(), photoRegionInfo.map.mapSize.$2.toDouble()),
                                                  markerPixel: Offset(px.toDouble(), py.toDouble()),
                                                  maxScale: 28,
                                                  initScale: 2,
                                                ),
                                              ),
                ...
                if(isFilter){
                  ...
                  if(filtration == Filtration.onlyDailyTask){
                    ...
                    for(final image in images){
                      if(cancel) break;
                      try{
                        await image.getAddition(game.selectedUid?.value, AlbumType.NikkiPhotos_HighQuality);
                      }catch(e){
                        e;
                      }finally{
                        current++;
