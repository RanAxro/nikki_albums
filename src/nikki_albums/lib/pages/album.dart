...
import 'package:nikki_albums/component/rui.dart';
...
bool _isPrimaryMouseDown = false;
                                ...
                                builder: (BuildContext context, AsyncSnapshot snapshot){
                                  if(snapshot.connectionState == ConnectionState.done && snapshot.hasData){
                                    ...
                                    return Listener(
                                      ...
                                      onPointerDown: (e){
                                        if(e.kind == PointerDeviceKind.mouse){
                                          ...
                                          if(e.buttons == kPrimaryMouseButton){
                                            _isPrimaryMouseDown = true;
                                            toggleSelect();
                                          }
                                          // 右键预览大图
                                          else if(e.buttons == kSecondaryButton){
                                            _viewImage.build(imgSnapshot.data!, index);
                                          }
                                        }
                                      },
                                      // 鼠标键松开
                                      onPointerUp: (e){
                                        if(e.kind == PointerDeviceKind.mouse){
                                          _isPrimaryMouseDown = false;
                                        }
                                      },
                                      child: MouseRegion(
                                        onEnter: (_){
                                          // 长按多选
                                          if(_isPrimaryMouseDown) toggleSelect();
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            _toolButton(isInTS, "删除", "assets/icon/delete.webp", () async{
                              ...
                              backupStationImage.value = {};
                              refreshAlbum.value++;
                            }, isAccident: true, alertText: "您确定要删除中转站的所有照片吗?删除后将永远无法恢复!", alertTrueText: "删除", alertFalseText: "不删除"),
                                          ...
                                          child: Stack(
                                            children: [
                                              IconButton(
                                                onPressed: () async{
                                                  print("click");
                                                  final isRun = await rui.alert.show(text: "你确定要解散分组吗", trueText: "确定", falseText: "取消");
                                                  if(isRun){
                                            ...
                                            onPointerDown: (e){
                                              if(e.kind == PointerDeviceKind.mouse){
                                                ...
                                                // 右键预览大图
                                                  ...
                                                  _viewImage.build(imageList, index);
                                                }
                                              }
                                            },
                                            // 鼠标键松开
...
///工具区按钮
Widget _toolButton(bool isCanUse, String message, String img, Function() trigger, {bool isAccident = false, String alertText = "", String alertTrueText = "", String  alertFalseText = ""}) {
  return Tooltip(
    message: isCanUse ? message : "",
    child: GestureDetector(
      ...
      child: IconButton(
        ...
        onPressed: () async{
          if(isAccident){
            final res = await rui.alert.show(text: alertText, trueText: alertTrueText, falseText: alertFalseText);
            if(res) trigger();
          }else{
            trigger();
          }
                      ...
                      child: Container(
                        ...
                        child: rui.MultiValueListenableBuilder(
                          ...
                          builder: (BuildContext context, List<Object?> values){
                            ...
                            final List<File> imageFileList = image.keys.map((path) => File(path)).toList();
                            ...
                            return GridView.builder(
                              ...
                              // addRepaintBoundaries: false,
                              itemBuilder: (BuildContext context, int index){
                                final String path = imageFileList[index].path;
                                return FutureBuilder(
                                  ...
                                  builder: (BuildContext context, AsyncSnapshot snapshot){
                                    if(snapshot.connectionState == ConnectionState.done && snapshot.hasData){
                                      return GestureDetector(
                                        ...
                                        //右键浏览大图
                                        onSecondaryTap: (){_viewImage.build(imageFileList, index);},
                                        child: Image(
                                          image: MemoryImage(snapshot.data!),
                                          fit: BoxFit.cover,
                                        ),
...
abstract class _viewImage{
  ...
  static Future<void> build(List imageFileList, int index) async{
    if(selector != null){
      close();
    }
    await init();

    final ImageViewerController imageViewerController = ImageViewerController();
        ...
        return Stack(
          children: [
            ...
            Positioned(
              ...
              bottom: topBarHeight,
              child: ImageViewer(
                imageCount: imageFileList.length,
                initIndex: index,
                controller: imageViewerController,
                imageBuilder: (context, index){
                  return Image.file(File(imageFileList[index].path), fit: BoxFit.contain,);
                },
              ),
            ),
            //
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        rui.block5W,
                        // 关闭
                        _toolButton(true, "关闭", "assets/icon/close.webp", (){
                          close();
                        }),
                        rui.block5W,
                        // TODO 图片信息
                        _toolButton(false, "信息", "assets/icon/more.webp", (){}),
                        rui.block5W,
                        // TODO 导出
                        _toolButton(false, "信息", "assets/icon/export.webp", (){}),
                        rui.block5W,
                        // 重置
                        _toolButton(true, "重置", "assets/icon/reset.webp", (){
                          imageViewerController.reset();
                        }),
                        rui.block5W,
                        // 水平翻转
                        _toolButton(true, "水平翻转", "assets/icon/horizontalFlip.webp", (){
                          imageViewerController.horizontalFlip();
                        }),
                        rui.block5W,
                        // 垂直翻转
                        _toolButton(true, "垂直翻转", "assets/icon/verticalFlip.webp", (){
                          imageViewerController.verticalFlip();
                        }),
                        rui.block5W,
                        // 逆时针旋转90°
                        _toolButton(true, "逆时针旋转90°", "assets/icon/rotateLeft90.webp", (){
                          imageViewerController.rotateLeft90();
                        }),
                        rui.block5W,
                        // 顺时针旋转90°
                        _toolButton(true, "顺时针旋转90°", "assets/icon/rotateRight90.webp", (){
                          imageViewerController.rotateRight90();
                        }),
                        rui.block5W,
                        // 上一张
                        _toolButton(true, "上一张", "assets/icon/back.webp", (){
                          imageViewerController.toPreviousImage();
                        }),
                        rui.block5W,
                        // 下一张
                        _toolButton(true, "下一张", "assets/icon/forward.webp", (){
                          imageViewerController.toNextImage();
                        }),
                      ],
                    ),
                  ),
